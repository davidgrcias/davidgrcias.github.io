...
const PortfolioStats = ({ isOpen, onClose }) => {
  ...
  const writeStoredStats = (updated) => {
    ...
    localStorage.setItem('webos-stats', JSON.stringify(updated));
  };
  ...
  useEffect(() => {
    const current = readStoredStats();
    const updated = { ...current, visits: (current.visits || 0) + 1 };
    writeStoredStats(updated);
    setStats(updated);

    // Track clicks
    const handleClick = () => {
      const base = readStoredStats();
      const nextStats = { ...base, clicks: (base.clicks || 0) + 1 };
  ...
  useEffect(() => {
    ...
    if (currentCount > 0) {
      setStats(prev => {
        const nextCount = Math.max(prev.appsOpened || 0, currentCount);
        if (nextCount === prev.appsOpened) return prev;
        const newStats = { ...prev, appsOpened: nextCount };
        writeStoredStats(newStats);
        return newStats;
      });
    }
  }, [windows]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const base = statsRef.current || readStoredStats();
      const newStats = {
        ...base,
        sessionTime,
        timeSpent: (base.timeSpent || 0) + 1,
      };
